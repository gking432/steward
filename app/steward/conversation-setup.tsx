...
export function ConversationSetup({workspace,today,onDone,manual=false}:{workspace:Workspace;today:string;onDone:(w:Workspace)=>void;manual?:boolean}){
 ...
 return <main className="sc-shell">
  ...
  <aside className="sc-plan"><p className="sc-eyebrow">Your draft · not yet confirmed</p><h2>Taking shape</h2>
   ...
   <a href="/demo?statements=1">Inspect the sample statements</a><h3>What matters to you</h3>{draft.goals.length?<ol>{draft.goals.map(g=><li key={g.id}><strong>{g.name}</strong><span>{g.amount!==null?formatMoney(g.amount):g.kind==='fund'?'Open-ended savings':'Price needed'}{g.date?` · ${g.date}`:''}<br/>{formatMoney(proposal?.lines.find(l=>l.claim.id===`claim:chat:${g.id}`||l.claim.name===g.name)?.amount??0)} this paycheck{g.contribution!==null&&g.contribution!==undefined?` · chosen contribution ${formatMoney(g.contribution)}`:''}</span></li>)}</ol>:<p>Your priorities will appear as we talk. Required bills stay protected.</p>}
