...
export function ConversationSetup({workspace,today,onDone,manual=false}:{workspace:Workspace;today:string;onDone:(w:Workspace)=>void;manual?:boolean}){
 ...
 function confirm(){
  ...
  const next=confirmProposal(preview,proposal);sessionStorage.removeItem('steward-chat:'+location.pathname);
 ...
 return <main className="sc-shell">
  ...
  <aside className="sc-plan"><p className="sc-eyebrow">Your draft · not yet confirmed</p><h2>Taking shape</h2>
   ...
   <h3>What matters to you</h3>{draft.goals.length?<ol>{draft.goals.map(g=><li key={g.id}><strong>{g.name}</strong><span>{g.amount!==null?formatMoney(g.amount):g.kind==='fund'?'Open-ended savings':'Price needed'}{g.date?` · ${g.date}`:''}</span></li>)}</ol>:<p>Your priorities will appear as we talk. Required bills stay protected.</p>}
