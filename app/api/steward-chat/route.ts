...
import { evaluatePurchase, buildPaydayProposal } from '../../../lib/model/decide';
...
const goalProperties={id:{type:'string'},name:{type:'string'},kind:{type:'string',enum:['fund','purchase','payoff']},amount:nullableNumber,contribution:nullableNumber,date:nullableString,accountId:nullableString,evidence:{type:'string'}};
...
export async function POST(request:Request){
 ...
 const plan=planCycle(workspace,today);const calculatedDraft=workspaceFromChat(workspace,today,draft,mode==='setup');const allocations=buildPaydayProposal(calculatedDraft,today);const started=Date.now();
 const result=await structuredConversation({today,mode,currentDraft:draft,goalAllocations:calculatedDraft.claims.filter(c=>c.status==='active').map(c=>({name:c.name,amountThisPaycheck:allocations?.lines.find(l=>l.claim.id===c.id)?.amount??0})),transcript:turns,facts:{paycheck:workspace.profile.takeHomePay,payFrequency:workspace.profile.payFrequency,nextPayday:workspace.profile.nextPayday,buckets:workspace.buckets,accounts:workspace.accounts.map(a=>({id:a.id,name:a.name,type:a.type})),goals:workspace.claims,calculatedPlan:plan?{income:plan.income,bills:plan.reservesTotal,spending:plan.spendTotal,freeCapacity:plan.freeCapacity}:null}},schema,instructions);
