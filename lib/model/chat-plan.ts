...
export const chatGoalSchema=z.object({id:z.string().min(1).max(80),name:z.string().min(1).max(100),kind:z.enum(['fund','purchase','payoff']),amount:money.nullable(),date:z.string().regex(/^\d{4}-\d{2}-\d{2}$/).nullable(),accountId:z.string().max(120).nullable(),evidence:z.string().min(1).max(1000)}).strict();
