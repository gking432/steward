...
import { billAmount, currentCycle, daysBetween, liquidCash } from './engine';
...
export function currentLiquidity(workspace: Workspace, today: string) {
  ...
  const obligations = workspace.buckets.filter(b => b.kind === 'reserve' && (!b.dueDate || !cycle || b.dueDate < cycle.end)).reduce((sum, b) => sum + billAmount(b), 0);
