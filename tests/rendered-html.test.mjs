...
test('demo begins before account connection or financial intake',async()=>{const html=await page('/demo');assert.match(html,/data-demo-phase="0"/);assert.match(html,/>Get started/);assert.doesNotMatch(html,/name="payday"/);assert.doesNotMatch(html,/id="steward-reply"/);});
