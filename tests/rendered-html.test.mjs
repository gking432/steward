...
test('full setup remains a distinct reachable route',async()=>{const html=await page('/demo');assert.match(html,/Explore a sample plan/);assert.match(html,/A little clarity/);assert.match(html,/Use the sample numbers/);assert.match(html,/Planning progress/);});
