...
test('full setup remains a distinct reachable route',async()=>{const html=await page('/demo');assert.match(html,/Sample data/);assert.match(html,/role="log"/);assert.match(html,/id="steward-reply"/);assert.match(html,/2,150/);assert.match(html,/1,600/);assert.doesNotMatch(html,/name="payday"/);assert.match(html,/Planning progress/);});
